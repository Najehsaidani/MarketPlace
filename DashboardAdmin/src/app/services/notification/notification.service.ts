...
export class NotificationService {
  private notificationsList: { message: string, type: string, id: number }[] = [];
  notifications$ = new BehaviorSubject<{ message: string, type: string, id: number }[]>([]);

  addNotification(message: string, type: string = 'info') {
    const notificationId = Date.now();
    this.notificationsList.push({ message, type, id: notificationId });
